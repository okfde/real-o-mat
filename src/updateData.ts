...
const renderMarkdown = (value: string | null | undefined, inline = false) =>
  value
    ? changeLinksToExternal(
        (inline
          ? markdown.parseInline(value)
          : markdown.parse(value)) as string,
      ).trim()
    : value
...
for (const election of elections) {
  ...
  const electionQuestions = questions
    .filter((q) => q.fields.Wahl.id === election.id)
    .map((question, index) => {
      return {
        id: `question-${index}`,
        index,
        ...
        keyword: question.fields.Stichwort,
        answers: answers
          .filter((a) => a.fields.These.id === question.id)
          .map((answer) => {
            return {
              party: partySlug(
                parties.find((p) => p.id === answer.fields.Partei.id),
              ),
  ...
  const partySlugs = new Set(
    electionQuestions.flatMap((q) => q.answers.map((a) => a.party)),
  )
  ...
  await fs
    .writeFile(
      `./src/data/elections/${electionData.slug}.yaml`,
      yaml.dump(electionData),
      'utf8',
    )
    .then(() =>
      console.log(`YAML file for ${electionData.slug} created successfully.`),
    )
    .catch((err) =>
      console.error(`Error writing YAML file for ${electionData.slug}:`, err),
    )
}
