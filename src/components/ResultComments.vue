    ...
    <Transition mode="out-in" :name="transitionName">
      <article :key="currentQuestionIndex">
        ...
        <ul class="mt-8 grid gap-4 md:grid-cols-2">
          <Disclosure as="li" default-open v-slot="{ open }" v-for="({ party, comment }, i) in currentQuestion.answers"
            :key="party" class="flex flex-col">
            <DisclosureButton
              ...
              <IconChevron aria-hidden="true" class="h-5 w-5 transform text-purple-900 motion-safe:transition-transform"
                :class="{
                  'rotate-180': open,
                }" />
            </DisclosureButton>
